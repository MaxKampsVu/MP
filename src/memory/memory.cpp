...
SC_MODULE(Memory) {
    ...
    void execute() {
        while (true) {
            ...
            uint64_t data = 0;
            if (f == FUNC_WRITE) {
                VERBOSE && cout << sc_time_stamp() << ": MEM received write" << endl;
                data = Port_Data.read().to_uint64();
            } else {
                VERBOSE && cout << sc_time_stamp() << ": MEM received read" << endl;
            }
            VERBOSE && cout << sc_time_stamp() << ": MEM address " << addr << endl;
        ...
        while (!tracefile_ptr->eof()) {
            ...
            if (tr_data.type != TraceFile::ENTRY_TYPE_NOP) {
                ...
                if (f == Memory::FUNC_WRITE) {
                    VERBOSE && cout << sc_time_stamp() << ": CPU sends write" << endl;
                    ...
                    VERBOSE && cout << sc_time_stamp() << ": CPU sends read" << endl;
                }
